...
| `coh/cc.c` | `#if Z8001` arms in the driver's variant defaults: `VSEG` + `VREADONLY` instead of `V80186`, and the model-selection block chooses `VLARGE` unless `-VSMALL` was asked for, in place of the Intel OMF rules | the C900's C model is the segmented one -- a pointer is a two-word seg:offset (`h/z8001/varmch.h` VSEG) -- and the donor's default is Intel's, so a driver taking it would compile every program for a flat Z8002 with truncated pointers. `VREADONLY` goes with it because the COHERENT 3.2 headers spell const `readonly` (`ctype.h`) and without the keyword they do not parse. The Intel block below forces the small model outside OMF output; there is no OMF on this machine, and `VSEG`/`VNSEG` are one machine-dependent slot apart and must never both be set. Both arms are `#if Z8001`, so no other target sees them |
| `n0/ddecl.c`, `h/cc0.h`, `n0/z8001/bind.c` | a parameter declared `char'/`unsigned char' KEEPS that type instead of being retyped to `int': `declare()`'s C_ARG arm asks the machine layer's new `subwordparm()` first, and skips `fixtype()` when it answers yes (`extern int subwordparm()` declared in cc0.h). The machine layer then binds such a parameter at the LOW-ORDER byte of its word slot -- `bindargs()` gives it `offset+1` and still advances by 2, so every later parameter keeps its address | THE ONE FRONT-END EDIT MADE ON PURPOSE, owner-decided (TASK #321), because no machine layer can express the fix: after `fixtype()` retypes the parameter to `int` the fact that it was a byte object is GONE, and `&c` can only name the whole word. K&R promotes a char ARGUMENT to int at the call, so the parameter does occupy a word (unchanged: callers still push one, and the ABI is untouched) -- but the parameter is still a one-byte object, and on this BIG-ENDIAN machine the caller's byte is the second byte of that word. The donor handed out the word's own address, which is the low-order byte only because the i8086 it descends from is little-endian; here it named the HIGH byte, zero for every ASCII character, so `write(fd,&c,1)` inside `f(c) char c;` wrote a NUL -- libcurses `waddch` drew NULs while the cursor advanced, and kermit could not send a character (#324). `subwordparm()` answers from `mysizes[]`, so a machine whose char is a word gets the donor's promotion unchanged, and short/float are unaffected (a short IS a word; float is promoted to double by both sides). Reading the parameter's value now takes a byte load plus the extend the declared type asks for, where the promoted word allowed one word load; that is the price of the object having an address at all, and it is also what K&R says the object is -- `f(c) char c; c = 300;` now truncates, and a caller passing 200 into a signed `char' parameter reads -56 back, both of which every conforming compiler does (the differential oracle, gcc, agrees on all 18 generators). Code that wants the wider value must say `int' or `unsigned char', as it must anywhere else. Measured cost: over the 315 objects of this compiler's own self-host build, the ONLY two that change are the two files edited here; over 108 COHERENT userland commands the total is +76 bytes, 0.014%, in exactly the five objects that have a byte parameter -- and `units.o' gets SMALLER, because a byte compare no longer widens first. A `register` byte parameter is not given a register, exactly as a `register char' auto is not (`grabreg()` takes word types only), so it stays addressable and correct. Six value assertions through cc0->cc1->cc2->ld->run cover both byte types, both directions and the second parameter's placement (tests/regress.sh, promoted from expected-fail); the byte-identity fixpoint cannot see this class at all, since a compiler reading the wrong byte agrees with itself about it |
| `n2/getfun.c`, `n2/cc2.c` | `freefunc()' releases the instruction list of the function just emitted; cc2.c calls it after genepilog() | the donor never frees an INS.  getfunc() starts each function's list at `ins' without looking back, so on the native C900 -- where cc2's heap is one 64 K data segment -- the peak is every instruction in the translation unit at once and a large module cannot be compiled at all.  The emit boundary is the safe place to free: once genfunc() has run nothing points into the list, because the fixups outcoh records name symbols and not instructions.  Found by Michal Pleban compiling this tree natively |
| `n2/optim.c` | `newlabel()' takes cfatal("out of space") on a failed malloc, in place of a NULL test whose `else' was missing | the donor initialises the record inside `if (lp != NULL)' and then returns `lp' regardless, so an exhausted heap threads a NULL INS into the list. On the C900 the dereference is a write through a 4-byte far pointer whose segment is whatever the register happened to hold -- there is no mapped page 0 to absorb it as there was on the donor's host -- and a 64 K heap is one a real compile can actually exhaust.  cfatal is the pass's own out-of-space exit (common/diag.c).  Found by Michal Pleban |
| `n0/get.c` | both `\r' tests in get() -- skipped at a line start, and skipped between a backslash and its newline -- are unconditional, not `#if GEMDOS || MSDOS' | the donor guarded them as a DOS concern, but a carriage return is nothing on every host: what the guard actually decides is whether a CR LF source preprocesses correctly.  It does not.  The second test is the load-bearing one -- with a CR between the backslash and the newline the splice is not recognised, so a multi-line macro or string silently becomes something else, with no diagnostic anywhere.  No file in this tree or in the kernel and userland it compiles carries a CR today, so nothing here changes; it is the C900 sources this compiler is aimed at next that are CR LF throughout (upstream's own kernel tree), and a compiler that miscompiles them quietly is worse than one that refuses them.  Found by Michal Pleban |
| `coh/cc.c` | `if (Eflag) setvariant(VCPP);' moved out of `#if TEMPBUF' to the head of resolve() | TEMPBUF is defined nowhere in this tree -- it guards a COH386 malloc efficiency hack -- so on the C900 driver that line never ran and -E did not set VCPP.  resolve() then reaches `if (qpass == NONE)' with VCPP clear and chooses qpass = LD, so `cc -E' preprocesses, compiles, assembles and LINKS instead of stopping after the preprocessor.  Whether a build buffers its temporary files has nothing to do with what -E means. Found by Michal Pleban |
<!-- END GENERATED BAKED -->
