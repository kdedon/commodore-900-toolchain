...
/*
 * Second flag word (TTY t_xflags).  t_flags has no spare bit.
 *
 * TX_OSTOP records that T_STOP was raised by a process asking for it with
 * TCXONC, as against the line discipline raising it for flow control.  The two
 * stop output identically -- ttstart() tests only T_STOP -- but they are
 * released by different things, so ttioctl()'s flow-control cleanup leaves a
 * TX_OSTOP stop standing and only TCXONC(TCOON) takes it down.
 */
#define	TX_OSTOP 0x0001		/* Output stopped by TCXONC, not by ^S */
