...
/*
 * Modes.
 * The same di_mode bits <sys/stat.h> spells S_IFMT, S_IFDIR, ... S_ISVTX,
 * under their on-disk names, for the kernel and for the utilities that read a
 * raw inode.  A program that has a struct stat wants the S_* spelling; only
 * one that walks dinodes wants these.  Nothing here may disagree with
 * <sys/stat.h>, which additionally names the group and other permission
 * triads the kernel derives by shifting these three bits.
 */
