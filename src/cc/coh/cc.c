...
main(argc, argv) int argc; char *argv[];
{
	 ...
	 * VNSEG is the Z8002's flat 16-bit model and is not what this system
	 * is compiled in, so the default must be VSEG -- a compiler that
	 * defaulted to VNSEG would build every program with truncated
	 * pointers.  VREADONLY goes with it: the system headers spell const
	 * `readonly' (ctype.h), so without the keyword they do not parse.
	 */
	setvariant(VSEG);
	setvariant(VREADONLY);
#else
	setvariant(V80186);
#endif
...
#define	NARENA	131072
	 ...
	 * default and here would silently compile every program for a Z8002.
	 * There is no OMF on this machine, so the choice is simply VSEG unless
	 * VNSEG was asked for, and asking for it clears the segmented bit --
	 * the two are one machine-dependent slot apart and must never both be
	 * set (see h/z8001/varmch.h).
	 */
	if (isvariant(VSMALL))
		clrvariant(VLARGE);
	else
