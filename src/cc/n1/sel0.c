...
register TREE *tp;
{
	...
	if (c == MFLOW) {
		...
		if (isrelop(op)) {
			rel = op;
			if (isnval(tp->t_rp, 0) && !islong(tp->t_lp->t_type) && tp->t_lp->t_op != QUEST) {
				if (rel == ULE)
					rel = EQ;
				else if (rel == UGT)
					rel = NE;
...
TREE	*ttp;
{
	...
	if (ttp->t_op>=INCBEF && ttp->t_op<=DECAFT) {
		...
		tp = ttp->t_lp;
	/* A store spills `tp' to a stack temp and enqueues `temp = tp'.  If
	 * selecting THAT assignment stores `tp' again, the second round has the
	 * same subtree and the same registers and must reach the same place: the
	 * list grows without converging, and NSTORE only says how long it takes.
	 * One subtree therefore gets one store, and a demand for a second is
	 * reported where it happens rather than as an exhausted table. */
	for (i = 0; i < nstorelist; i += 1)
		if (storesub[i] == tp)
			cbotch("store does not converge, op=%d", tp->t_op);
	sp = tp;
	ap = tempnode(tp, 1);
	if (sideflag == MRVALUE)
		ap = leafnode(ap);
