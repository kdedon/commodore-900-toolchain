...
again:
	...
	case SHL:
		...
		&& (isuns(rp->t_type)==0 || rp->t_ival>=0)) {
			tp->t_op += ADD-SUB;
			/* the node keeps the subtract's own type: its signedness
			 * (and pointer-ness) selects the form in consumers */
			rp->t_ival = -rp->t_ival;
			amd(rp);
			tp->t_lp  = lp;
			tp->t_rp  = rp;
	...
	case ASHR:
		...
		if (isnval(rp, 0)) {
			...
			/*
			 * The value of `x op= 0' is x at the node's PROMOTED
			 * type, so a narrower lvalue is CONVERTed to it.
			 * Retyping the lvalue in place widens the object's own
			 * load instead, and `char c; v = (c += 0)' then reads
			 * the neighbouring byte.  An effect context reads
			 * nothing, and keeps the retype.
			 */
			if (c != MEFFECT && isbyte(lp->t_type) && !isbyte(tt))
				lp = leftnode(CONVERT, lp, tt, ts);
			else {
				lp->t_type = tt;
				lp->t_size = ts;
			}
