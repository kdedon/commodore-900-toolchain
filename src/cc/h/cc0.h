/*
 * h/cc0.h
 * Definitions for the C compiler parser (cc0)
 * and the C compiler preprocessor (cpp).
 * Includes the definitions of the reserved words,
...
/*
 * Define the sizes of the tables in the parser.
 * These can be changed to just about anything.
 */
 ...
 * Each unique token is the head of a list of symbol buckets.
 */
struct token {
	TOK	*t_tp;
	SYM	*t_sym;
...
/* Macros. */
#define	ideq(tp)	(strcmp((tp)->t_id, id) == 0)
