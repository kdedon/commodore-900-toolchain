...
fold0(op, lp, rp) int op; TREE *lp, *rp;
{
	...
	/*
	 * Grab the left and right values as [u]lval_t.  Each operand is
	 * widened according to ITS OWN signedness, not the goal type's: an
	 * unsigned operand promoted into a wider signed goal (e.g. an
	 * unsigned int added to a long) is value-preserving and must zero
	 * extend, while a signed operand promoted into an unsigned goal must
	 * sign extend.  Keying the extension off the goal type mis-widens
	 * either case.  The operation's own signed/unsigned semantics
	 * (DIV, REM, SHR, relations below) still use the goal-type `uflag'.
	 */
	lv = grablval(lp, lp->t_type == T_UINT || lp->t_type == T_ULONG);
	if (rp != NULL)
		rv = grablval(rp, rp->t_type == T_UINT || rp->t_type == T_ULONG);
