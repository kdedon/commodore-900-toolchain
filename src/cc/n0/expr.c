...
term()
{
	...
	case LPAREN:
		...
		if ((cp = cast()) != NULL) {
			...
			if (bitcompat(tt, lt)
			 && !((cp->t_lp->t_op==DIV||cp->t_lp->t_op==REM||cp->t_lp->t_op==SHR||cp->t_lp->t_op==ASHR||cp->t_lp->t_op==ADIV||cp->t_lp->t_op==AREM)
			   && ((tt==T_UCHAR||tt==T_USHORT||tt==T_UINT||tt==T_ULONG)
			    != (lt==T_UCHAR||lt==T_USHORT||lt==T_UINT||lt==T_ULONG)))) {
				tp = cp->t_lp;
	...
	case CALL:
		if ((dp = lp->t_dp) != NULL && dp->d_type == D_FUNC)
			gt.t_dp = dp->d_dp;
		      ...
		      && dp->d_type == D_PTR
		      && dp->d_dp != NULL && dp->d_dp->d_type == D_FUNC) {
		...
		if ((cv & (GTL | GTR)) != 0) {
			...
			if ((cv & (CVL | CVR | CVRA)) != 0) {
				...
				if (gtp->t_dp != NULL
				 ...
				 && gtp->t_dp->d_dp != NULL && gtp->t_dp->d_dp->d_type == D_FUNC)
					gt.t_rp = bzcon(0);
				else
