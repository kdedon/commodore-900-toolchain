...
loop:
	...
	case SWITCH:
		...
		tp = pexpr();
		tt = tltype(tp);
		if ((tt == T_LONG  && mytypes[T_INT]  == mytypes[T_LONG])
		 || (tt == T_ULONG && mytypes[T_UINT] == mytypes[T_ULONG])) {
			if (isvariant(VSBOOK))
				cstrict("switch expression is long");
		} else if (tt > T_UINT)
	...
	case RETURN:
		...
		if (s != SEMI) {
			...
			if (bitcompat(tltype(lp), tltype(tp))
			 && !((tp->t_op==DIV||tp->t_op==REM||tp->t_op==SHR||tp->t_op==ASHR||tp->t_op==ADIV||tp->t_op==AREM)
			   && ((tltype(tp)==T_UCHAR||tltype(tp)==T_USHORT||tltype(tp)==T_UINT||tltype(tp)==T_ULONG)
			    != (lp->t_type==T_UCHAR||lp->t_type==T_USHORT||lp->t_type==T_UINT||lp->t_type==T_ULONG))))
...
TREE	*tp;
{
	...
	if (isvariant(VSCCON)) {
		/* Catch typing error "if (i = 1) ..." for "if (i == 1)...". */
		while (op == ASSIGN) {
			tp = tp->t_rp;
			op = tp->t_op;
		}
