...
struct nlist *nlp;
{
	...
	struct ldheader lh;
	struct ldsym ste;
	fsize_t symsize;	/* a file span, so it must hold more than 64K:
				 * size_t here is a 16-bit unsigned and the
				 * header+text+data span of a binary over 64K
				 * (the kernel) does not fit it. */
	register int n;
