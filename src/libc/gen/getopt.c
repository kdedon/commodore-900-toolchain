 ...
 * getopt()
 *
 * De-ANSI'd from the 4.2 libc for this K&R compiler: the USE_PROTO halves of
 * each declaration are gone, LOCAL is static and CONST is nothing, so
 * <sys/compat.h> -- which reaches into the 4.x common/ include tree this target
 * does not have -- is not needed.
 */
